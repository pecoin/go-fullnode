// Package pecstats implements the network stats reporting service.
package pecstats
...
import (
	...
	ethproto "github.com/pecoin/go-fullnode/pec/protocols/eth"
...
// backend encompasses the bare-minimum functionality needed for pecstats reporting
...
func (s *Service) login(conn *connWrapper) error {
	...
	var network string
	if info := infos.Protocols["pec"]; info != nil {
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the pecstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to pecstats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to pecstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to pecstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to pecstats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to pecstats")
