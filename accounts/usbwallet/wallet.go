...
type wallet struct {
	...
	deriveChain     fullnode.ChainStateReader // Blockchain state reader to discover used account with
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain fullnode.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
func (w *wallet) SignTx(account accounts.Account, tx *types.Transaction, chainID *big.Int) (*types.Transaction, error) {
	...
	if sender != account.Address {
		...
		return nil, fmt.Errorf("signer mismatch: expected %s, got %s", account.Address.Base58(), sender.Base58())
	}
	return signed, nil
}
