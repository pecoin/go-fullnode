...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.ethstats == "" {
		log.Error("No pecstats server configured")
		return
	}
	...
	if err != nil {
		if boot {
			infos = &nodeInfos{port: 21861, peersTotal: 512, peersLight: 256}
		} else {
			infos = &nodeInfos{port: 21861, peersTotal: 50, peersLight: 0, gasTarget: 7.5, gasLimit: 10, gasPrice: 1}
		}
	}
	existed := err == nil
			...
			if infos.etherbase == "" {
				...
				for {
					if address := w.readAddress(); address != nil {
						...
						infos.etherbase = address.Base58()
						break
					}
				}
			} else {
				...
				infos.etherbase = w.readDefaultAddress(common.HexToAddress(infos.etherbase)).Base58()
			}
