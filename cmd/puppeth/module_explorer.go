...
RUN \
  echo 'fullnode --cache 512 init /genesis.json' > explorer.sh && \
  echo $'fullnode --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.EthPort}} --bootnodes {{.Bootnodes}} --pecstats \'{{.Ethstats}}\' --cache=512 --http --http.api "net,web3,pec,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" --exitwhensynced' >> explorer.sh && \
  echo $'exec fullnode --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.EthPort}} --bootnodes {{.Bootnodes}} --pecstats \'{{.Ethstats}}\' --cache=512 --http --http.api "net,web3,pec,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" &' >> explorer.sh && \
