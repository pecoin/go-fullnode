...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"pecstats", "explorer", "wallet", "faucet"} {
	...
	// If we have pecstats running, ask whether to make the secret public or not
	if w.conf.ethstats != "" {
		fmt.Println()
		fmt.Println("Include pecstats secret on dashboard (y/n)? (default = yes)")
