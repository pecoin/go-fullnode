...
EXPOSE 8080 21861 21861/udp

ENTRYPOINT [ \
	"faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--pecstats", "{{.Ethstats}}", "--ethport", "{{.EthPort}}",     \
...
func (info *faucetInfos) Report() map[string]string {
	...
	if info.node.keyJSON != "" {
		...
		if err := json.Unmarshal([]byte(info.node.keyJSON), &key); err == nil {
			...
			report["Funding account"] = common.HexToAddress(key.Address).Base58()
		} else {
