...
// ethstatsDockerfile is the Dockerfile required to build an pecstats backend
// and associated monitoring site.
var ethstatsDockerfile = `
FROM puppeth/pecstats:latest

RUN echo 'module.exports = {trusted: [{{.Trusted}}], banned: [{{.Banned}}], reserved: ["yournode"]};' > lib/utils/config.js
`

// ethstatsComposefile is the docker-compose.yml file required to deploy and
// maintain an pecstats monitoring site.
var ethstatsComposefile = `
version: '2'
services:
  pecstats:
    build: .
    image: {{.Network}}/pecstats
...
// deployEthstats deploys a new pecstats container to a remote machine via SSH,
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	// Build and deploy the pecstats service
	if nocache {
...
// ethstatsInfos is returned from an pecstats status check to allow reporting
...
// checkEthstats does a health-check against an pecstats server to verify whether
// it's running, and if yes, gathering a collection of useful infos about it.
func checkEthstats(client *sshClient, network string) (*ethstatsInfos, error) {
	// Inspect a possible pecstats container on the host
