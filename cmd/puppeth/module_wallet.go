...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'fullnode --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec fullnode --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --pecstats \'{{.Ethstats}}\' --cache=512 --http --http.addr=0.0.0.0 --http.corsdomain "*" --http.vhosts "*"' >> wallet.sh
