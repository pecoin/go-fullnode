...
{{end}}
RUN \
  echo 'fullnode --cache 512 init /genesis.json' > fullnode.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.ethereum/keystore/ && cp /signer.json /root/.ethereum/keystore/' >> fullnode.sh && \{{end}}
	echo $'exec fullnode --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --pecstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> fullnode.sh

ENTRYPOINT ["/bin/sh", "fullnode.sh"]
`
	...
	if info.gasTarget > 0 {
		...
		if info.keyJSON != "" {
			...
			if err := json.Unmarshal([]byte(info.keyJSON), &key); err == nil {
				...
				report["Signer account"] = common.HexToAddress(key.Address).Base58()
			} else {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 fullnode --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
