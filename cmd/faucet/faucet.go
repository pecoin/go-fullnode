...
var (
	...
	ethPortFlag = flag.Int("ethport", 21861, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Ethereum protocol")
	statsFlag   = flag.String("pecstats", "", "Ethstats network monitoring auth string")
...
type faucet struct {
	...
	client *peclient.Client    // Client connection to the Ethereum chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the pecstats monitoring and reporting service'
	if stats != "" {
		if err := pecstats.New(stack, lesBackend.ApiBackend, lesBackend.Engine(), stats); err != nil {
			return nil, err
		}
	}
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/pecoin/go-fullnode/issues")
		}
		if err != nil {
			if err = sendError(wsconn, err); err != nil {
		...
		if err = sendSuccess(wsconn, fmt.Sprintf("Funding request accepted for %s into %s", username, address.Base58())); err != nil {
...
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	address := common.Base58ToAddress(string(regexp.MustCompile("[a-zA-Z0-9]{27,34}").Find(body)))
...
func authNoAuth(url string) (string, string, common.Address, error) {
	...
	return address.Base58() + "@noauth", "", address, nil
}
