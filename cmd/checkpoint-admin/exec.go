...
func deploy(ctx *cli.Context) error {
	...
	for _, account := range strings.Split(ctx.String(signersFlag.Name), ",") {
		...
		addrs = append(addrs, common.Base58ToAddress(account))
	}
...
func sign(ctx *cli.Context) error {
	...
	if !ctx.GlobalIsSet(nodeURLFlag.Name) {
		...
		address = common.Base58ToAddress(ctx.String(oracleFlag.Name))
	} else {
		...
		head, err := peclient.NewClient(node).HeaderByNumber(reqCtx, nil)
		if err != nil {
			return err
		}
	...
	isAdmin := func(addr common.Address) error {
		...
		return fmt.Errorf("signer %v is not the admin", addr.Base58())
	}
	...
	if !offline {
		...
		//	return err
		//}
		if err := isAdmin(common.Base58ToAddress(signer)); err != nil {
			return err
		}
	}
...
func publish(ctx *cli.Context) error {
	...
	head, err := peclient.NewClient(client).HeaderByNumber(reqCtx, nil)
	if err != nil {
		return err
	}
	num := head.Number.Uint64()
	recent, err := peclient.NewClient(client).HeaderByNumber(reqCtx, big.NewInt(int64(num-128)))
	if err != nil {
		return err
	}
	...
	for i, sig := range sigs {
		...
		fmt.Printf("Signer %d => %s\n", i+1, ecrecover(sighash, sig).Base58())
	}
	fmt.Println()
