...
import (
	...
	"github.com/pecoin/go-fullnode/common/fdlimit"
	"github.com/pecoin/go-fullnode/consensus"
	"github.com/pecoin/go-fullnode/consensus/clique"
	...
	"github.com/pecoin/go-fullnode/graphql"
	"github.com/pecoin/go-fullnode/internal/ethapi"
	"github.com/pecoin/go-fullnode/internal/flags"
	"github.com/pecoin/go-fullnode/les"
	"github.com/pecoin/go-fullnode/log"
	"github.com/pecoin/go-fullnode/metrics"
	"github.com/pecoin/go-fullnode/metrics/exp"
	"github.com/pecoin/go-fullnode/metrics/influxdb"
	"github.com/pecoin/go-fullnode/miner"
	...
	"github.com/pecoin/go-fullnode/pec"
	"github.com/pecoin/go-fullnode/pec/downloader"
	"github.com/pecoin/go-fullnode/pec/ethconfig"
	"github.com/pecoin/go-fullnode/pec/gasprice"
	"github.com/pecoin/go-fullnode/pec/tracers"
	"github.com/pecoin/go-fullnode/pecdb"
	"github.com/pecoin/go-fullnode/pecstats"
...
var (
	...
	TestnetFlag = cli.BoolFlag{
		Name:  "testnet",
		Usage: "Test network: pre-configured proof-of-authority test network",
	}
	...
	EthStatsURLFlag = cli.StringFlag{
		Name:  "pecstats",
		Usage: "Reporting URL of a pecstats service (nodename:secret@host:port)",
	}
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `fullnode account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func setDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.GlobalBool(TestnetFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		...
		if _, err := os.Stat(legacyPath); !os.IsNotExist(err) {
			log.Warn("Using the deprecated `testnet` datadir.")
...
func setTxPool(ctx *cli.Context, cfg *core.TxPoolConfig) {
	if ctx.GlobalIsSet(TxPoolLocalsFlag.Name) {
		...
		for _, account := range locals {
			...
			if trimmed := strings.TrimSpace(account); !common.IsBase58Address(trimmed) {
				Fatalf("Invalid account in --txpool.locals: %s", trimmed)
			} else {
				// cfg.Locals = append(cfg.Locals, common.HexToAddress(account))
				cfg.Locals = append(cfg.Locals, common.Base58ToAddress(account))
			}
		}
	}
...
// SetEthConfig applies pec-related command line flags to the config.
...
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := pecstats.New(stack, backend, backend.Engine(), url); err != nil {
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "fullnode.", tagsMap)
		}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) pecdb.Database {
	var (
		cache   = ctx.GlobalInt(CacheFlag.Name) * ctx.GlobalInt(CacheDatabaseFlag.Name) / 100
		handles = MakeDatabaseHandles()

		err     error
		chainDb pecdb.Database
	)
...
func MakeChain(ctx *cli.Context, stack *node.Node, readOnly bool) (chain *core.BlockChain, chainDb pecdb.Database) {
	var err error
	chainDb = MakeChainDatabase(ctx, stack)
...
// new format.
//
// e.g. fullnode account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// fullnode --keystore /tmp/mykeystore --lightkdf account new
//
