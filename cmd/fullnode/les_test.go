...
func (g *gethrpc) waitSynced() {
	...
	sub, err := g.rpc.Subscribe(context.Background(), "pec", ch, "syncing")
	if err != nil {
...
func startGethWithIpc(t *testing.T, name string, args ...string) *gethrpc {
	ipcName := fmt.Sprintf("fullnode-%d.ipc", atomic.AddUint32(&nextIPC, 1))
...
func initGeth(t *testing.T) string {
	...
	t.Logf("Initializing fullnode: %v ", args)
	g := runGeth(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
...
func startLightServer(t *testing.T) *gethrpc {
	datadir := initGeth(t)
	t.Logf("Importing keys to fullnode")
