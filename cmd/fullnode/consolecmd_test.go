...
const (
	ipcAPIs  = "admin:1.0 debug:1.0 pec:1.0 ethash:1.0 miner:1.0 net:1.0 personal:1.0 rpc:1.0 txpool:1.0 web3:1.0"
	httpAPIs = "pec:1.0 net:1.0 rpc:1.0 web3:1.0"
)
...
func TestConsoleWelcome(t *testing.T) {
	...
	// Start a fullnode console, make sure it's cleaned up and terminate the console
...
func TestAttachWelcome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		ipc = `\\.\pipe\gpec` + strconv.Itoa(trulyRandInt(100000, 999999))
	} else {
		ws := tmpdir(t)
		defer os.RemoveAll(ws)
		ipc = filepath.Join(ws, "fullnode.ipc")
	}
...
func testAttachWelcome(t *testing.T, geth *testgeth, endpoint, apis string) {
	// Attach to a running fullnode note and terminate immediately
