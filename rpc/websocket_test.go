...
func TestClientWebsocketPing(t *testing.T) {
	...
	// This is important for reproducing https://github.com/pecoin/go-fullnode/issues/19798
	<-ctx.Done()
	close(sendPing)
...
func wsPingTestHandler(t *testing.T, conn *websocket.Conn, shutdown, sendPing <-chan struct{}) {
	...
	const (
		...
		subNotify = `{"jsonrpc":"2.0","method":"pec_subscription","params":{"subscription":"0x00","result":1}}`
	)
