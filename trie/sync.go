...
type Sync struct {
	database pecdb.KeyValueReader     // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database pecdb.KeyValueReader, callback LeafCallback, bloom *SyncBloom) *Sync {
	ts := &Sync{
		database: database,
...
// both pec/6x style fast sync and snap/1x style state sync, the paths of trie
...
func (s *Sync) Commit(dbw pecdb.Batch) error {
