...
// the pec.BlockFetcher as the underlying fetcher but adding more additional
...
func newLightFetcher(chain *light.LightChain, engine consensus.Engine, peers *serverPeerSet, ulc *ulc, chaindb pecdb.Database, reqDist *requestDistributor, syncFn func(p *serverPeer)) *lightFetcher {
...
//
// Note, we rely on the underlying pec/fetcher to retrieve and validate the
// response, so that we have to obey the rule of pec/fetcher which only accepts
