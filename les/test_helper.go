...
import (
	...
	"github.com/pecoin/go-fullnode/accounts/abi/bind/backends"
	...
	"github.com/pecoin/go-fullnode/contracts/checkpointoracle/contract"
	...
	"github.com/pecoin/go-fullnode/les/flowcontrol"
...
func testIndexers(db pecdb.Database, odr light.OdrBackend, config *light.IndexerConfig, disablePruning bool) []*core.ChainIndexer {
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db pecdb.Database, peers *serverPeerSet, ulcServers []string, ulcFraction int) *clientHandler {
	var (
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db pecdb.Database, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend) {
	var (
		gspec = core.Genesis{
			Config:   params.AllEthashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
