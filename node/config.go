...
import (
	...
	"github.com/pecoin/go-fullnode/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of fullnode is "fullnode". If no
...
func (c *Config) NodeName() string {
	...
	if name == "fullnode" || name == "fullnode-testnet" {
		name = "Geth"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "fullnode" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by fullnode 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'fullnode' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
