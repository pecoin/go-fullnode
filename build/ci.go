...
import (
	...
	"github.com/pecoin/go-fullnode/crypto/signify"
	"github.com/pecoin/go-fullnode/internal/build"
	"github.com/pecoin/go-fullnode/params"
)

var (
	// Files that end up in the fullnode*.zip archive.
	gethArchiveFiles = []string{
		"COPYING",
		executablePath("fullnode"),
	}

	// Files that end up in the fullnode-alltools*.zip archive.
...
func doArchive(cmdline []string) {
	...
	var (
		...
		geth     = "fullnode-" + basegeth + ext
		alltools = "fullnode-alltools-" + basegeth + ext
	)
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with fullnode-release.pub"
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "fullnode-ci")`)
...
func downloadGo(goarch, goos, cachedir string) string {
	...
	godir := filepath.Join(ucache, fmt.Sprintf("fullnode-go-%s-%s-%s", dlgoVersion, goos, goarch))
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "fullnode-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "fullnode.exe" {
			gethTool = file
		} else {
	...
	// first section contains the fullnode binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Geth":     gethTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.fullnode.nsi", filepath.Join(*workdir, "fullnode.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("fullnode-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "fullnode.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.ethereum", "-v", "github.com/pecoin/go-fullnode/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("fullnode.aar", filepath.Join(GOBIN, "fullnode.aar"))
		os.Rename("fullnode-sources.jar", filepath.Join(GOBIN, "fullnode-sources.jar"))
		return
	}
	...
	archive := "fullnode-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("fullnode.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/pecoin/go-fullnode/mobile")
	...
	archive := "fullnode-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
