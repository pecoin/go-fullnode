...
import (
	...
	"github.com/pecoin/go-fullnode/core/state/snapshot"
...
type BlockChain struct {
	...
	db     pecdb.Database // Low level persistent database to store final content in
...
// Processor.
func NewBlockChain(db pecdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool, txLookupLimit *uint64) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (bc *BlockChain) SetHeadBeyondRoot(head uint64, root common.Hash) (uint64, error) {
	...
	updateFn := func(db pecdb.KeyValueWriter, header *types.Header) (uint64, bool) {
	...
	delFn := func(db pecdb.KeyValueWriter, hash common.Hash, num uint64) {
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - pecdb.IdealBatchSize)
			}
...
func (bc *BlockChain) reportBlock(block *types.Block, receipts types.Receipts, err error) {
	...
	for i, receipt := range receipts {
		...
		receiptString += fmt.Sprintf("\t %d: cumulative: %v gas: %v contract: %v status: %v tx: %v logs: %v bloom: %x state: %x\n",
			i, receipt.CumulativeGasUsed, receipt.GasUsed, receipt.ContractAddress.Base58(),
			receipt.Status, receipt.TxHash.Hex(), receipt.Logs, receipt.Bloom, receipt.PostState)
	}
