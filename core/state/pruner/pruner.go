...
func NewPruner(db pecdb.Database, headHeader *types.Header, datadir, trieCachePath string, bloomSize uint64) (*Pruner, error) {
...
func prune(maindb pecdb.Database, stateBloom *stateBloom, middleStateRoots map[common.Hash]struct{}, start time.Time) error {
...
// in the disk.
func RecoverPruning(datadir string, db pecdb.Database, trieCachePath string) error {
...
func extractGenesis(db pecdb.Database, stateBloom *stateBloom) error {
...
func getHeadHeader(db pecdb.Database) (*types.Header, error) {
...
Check the command description "fullnode snapshot prune-state --help" for more details.
`
