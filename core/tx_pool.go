...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	if pool.currentState.GetBalance(from).Cmp(tx.Cost()) < 0 {
		// log.Info(fmt.Sprintf("from: %s, balance: %s.", from.Hex(), pool.currentState.GetBalance(from)))
		log.Info(fmt.Sprintf("from: %s, balance: %s.", from.Base58(), pool.currentState.GetBalance(from)))
