...
func (st *StateTransition) buyGas() error {
	...
	if have, want := st.state.GetBalance(st.msg.From()), mgval; have.Cmp(want) < 0 {
		...
		return fmt.Errorf("%w: address %v have %v want %v", ErrInsufficientFunds, st.msg.From().Base58(), have, want)
	}
...
func (st *StateTransition) preCheck() error {
	...
	if st.msg.CheckNonce() {
		...
		if msgNonce := st.msg.Nonce(); stNonce < msgNonce {
			...
			return fmt.Errorf("%w: address %v, tx: %d state: %d", ErrNonceTooHigh,
				st.msg.From().Base58(), msgNonce, stNonce)
...
func (st *StateTransition) TransitionDb() (*ExecutionResult, error) {
	...
	if msg.Value().Sign() > 0 && !st.evm.Context.CanTransfer(st.state, msg.From(), msg.Value()) {
		...
		return nil, fmt.Errorf("%w: address %v", ErrInsufficientFundsForTransfer, msg.From().Base58())
	}
