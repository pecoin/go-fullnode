...
func ReadDatabaseVersion(db pecdb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db pecdb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db pecdb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db pecdb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func PushUncleanShutdownMarker(db pecdb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db pecdb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
