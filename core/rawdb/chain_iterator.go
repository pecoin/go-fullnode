...
func InitDatabaseFromFreezer(db pecdb.Database) {
	...
	for i := uint64(0); i < frozen; i++ {
		...
		if batch.ValueSize() > pecdb.IdealBatchSize {
...
func iterateTransactions(db pecdb.Database, from uint64, to uint64, reverse bool, interrupt chan struct{}) chan *blockTxHashes {
...
// signal received.
func indexTransactions(db pecdb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
...
// There is a passed channel, the whole procedure will be interrupted if any
// signal received.
func IndexTransactions(db pecdb.Database, from uint64, to uint64, interrupt chan struct{}) {
...
func indexTransactionsForTesting(db pecdb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
...
// There is a passed channel, the whole procedure will be interrupted if any
// signal received.
func unindexTransactions(db pecdb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
	// short circuit for invalid range
	if from >= to {
		return
	}
...
// There is a passed channel, the whole procedure will be interrupted if any
// signal received.
func UnindexTransactions(db pecdb.Database, from uint64, to uint64, interrupt chan struct{}) {
...
func unindexTransactionsForTesting(db pecdb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
