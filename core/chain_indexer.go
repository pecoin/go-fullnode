...
type ChainIndexer struct {
	chainDb  pecdb.Database      // Chain database to index the data from
	indexDb  pecdb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb pecdb.Database, indexDb pecdb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
