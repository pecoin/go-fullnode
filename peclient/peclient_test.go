...
var (
	_ = fullnode.ChainReader(&Client{})
	_ = fullnode.TransactionReader(&Client{})
	_ = fullnode.ChainStateReader(&Client{})
	_ = fullnode.ChainSyncReader(&Client{})
	_ = fullnode.ContractCaller(&Client{})
	_ = fullnode.GasEstimator(&Client{})
	_ = fullnode.GasPricer(&Client{})
	_ = fullnode.LogFilterer(&Client{})
	_ = fullnode.PendingStateReader(&Client{})
	// _ = fullnode.PendingStateEventer(&Client{})
	_ = fullnode.PendingContractCaller(&Client{})
)
...
func TestToFilterArg(t *testing.T) {
	...
	for _, testCase := range []struct {
		name   string
		input  fullnode.FilterQuery
		output interface{}
		err    error
	}{
		{
			"without BlockHash",
			fullnode.FilterQuery{
				Addresses: addresses,
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	ethservice, err := pec.New(n, config)
	if err != nil {
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == fullnode.NotFound {
		t.Fatal("error should not be nil/notfound")
	}
	// Test tx in block not found
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 1); err != fullnode.NotFound {
