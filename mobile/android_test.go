...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "fullnode-android-")
	if err != nil {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/pecoin/go-fullnode/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "fullnode.aar"), "fullnode.aar")
