...
func (a *Address) SetBase58(address string) error {
	addr, err := base58.Decode(address)
	if err != nil {
		return fmt.Errorf("base58 decode filed, address: %s", address)
	}
	if length := len(addr); length != common.AddressLength {
		return fmt.Errorf("invalid address length: %v != %v", length, common.AddressLength)
	}
	copy(a.address[:], addr)
	return nil
...
func (a *Address) GetBase58() string {
	return a.address.Base58()
}
