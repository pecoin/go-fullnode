...
import (
	...
	"github.com/pecoin/go-fullnode/accounts/scwallet"
...
func (s *PublicTxPoolAPI) Inspect() map[string]map[string]map[string]string {
	...
	var format = func(tx *types.Transaction) string {
		if to := tx.To(); to != nil {
			...
			return fmt.Sprintf("%s: %v wei + %v gas × %v wei", tx.To().Base58(), tx.Value(), tx.Gas(), tx.GasPrice())
		}
...
//
// https://github.com/pecoin/go-fullnode/wiki/Management-APIs#personal_sign
...
//
// https://github.com/pecoin/go-fullnode/wiki/Management-APIs#personal_ecRecover
...
// Unpair deletes a pairing between wallet and fullnode.
...
func DoCall(ctx context.Context, b Backend, args CallArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides map[common.Address]account, vmCfg vm.Config, timeout time.Duration, globalGasCap uint64) (*core.ExecutionResult, error) {
	...
	for addr, account := range overrides {
		...
		if account.State != nil && account.StateDiff != nil {
			...
			return nil, fmt.Errorf("account %s has both 'state' and 'stateDiff'", addr.Base58())
		}
