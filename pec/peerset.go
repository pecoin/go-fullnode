...
var (
	...
	// snap protocol without advertizing the pec main protocol.
	errSnapWithoutEth = errors.New("peer connected on snap without compatible pec support")
)

// peerSet represents the collection of active peers currently participating in
// the `pec` protocol, with or without the `snap` extension.
type peerSet struct {
	peers     map[string]*ethPeer // Peers connected on the `pec` protocol
	snapPeers int                 // Number of `snap` compatible peers for connection prioritization

	snapWait map[string]chan *snap.Peer // Peers connected on `pec` waiting for their snap extension
	snapPend map[string]*snap.Peer      // Peers connected on the `snap` protocol, but not yet on `pec`
...
// registerSnapExtension unblocks an already connected `pec` peer waiting for its
// `snap` extension, or if no such peer exists, tracks the extension for the time
// being until the `pec` main protocol starts looking for it.
func (ps *peerSet) registerSnapExtension(peer *snap.Peer) error {
	// Reject the peer if it advertises `snap` without `pec` as `snap` is only a
	// satellite protocol meaningful with the chain selection of `pec`
	...
	// Inject the peer into an `pec` counterpart is available, otherwise save for later
...
// registerPeer injects a new `pec` peer into the working set, or returns an error
...
// len returns if the current number of `pec` peers in the set. Since the `snap`
// peers are tied to the existence of an `pec` connection, that will always be a
// subset of `pec`.
