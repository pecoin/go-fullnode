...
// on the `pec` protocol and convert them into a more easily testable form.
...
func (h *testEthHandler) Handle(peer *eth.Peer, packet eth.Packet) error {
	...
	default:
		panic(fmt.Sprintf("unexpected pec packet type in tests: %T", packet))
	}
}
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	for len(seen) < len(insert) {
		...
		case 63, 64:
			select {
			case <-anns:
				t.Errorf("tx announce received on pre pec/65")
			case txs := <-bcasts:
				for _, tx := range txs {
		...
		case 65:
			...
			case <-bcasts:
				t.Errorf("initial tx broadcast received on post pec/65")
			}
...
// Tests that post pec protocol handshake, clients perform a mutual checkpoint
