...
type downloadTester struct {
	...
	stateDb pecdb.Database // Database used by the tester for syncing from peers
	peerDb  pecdb.Database // Database of the peers containing all data
...
func testSyncProgress(t *testing.T, protocol uint, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", fullnode.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", fullnode.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", fullnode.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", fullnode.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want fullnode.SyncProgress) {
	// Mark this method as a helper to report errors at callsite, not in here
	t.Helper()
...
func testForkedSyncProgress(t *testing.T, protocol uint, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", fullnode.SyncProgress{
