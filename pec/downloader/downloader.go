...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [pec/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB    pecdb.Database  // Database to state sync into (and deduplicate via)
...
func New(checkpoint uint64, stateDb pecdb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() fullnode.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "pec", p.version, "head", hash, "td", td, "mode", mode)
