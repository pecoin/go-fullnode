...
import (
	...
	"github.com/pecoin/go-fullnode/pec/fetcher"
...
type handlerConfig struct {
	Database   pecdb.Database            // Database for direct sync insertions
...
// runEthPeer registers an pec peer into the joint pec/snap peerset, adds it to
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	if err := h.downloader.RegisterPeer(peer.ID(), peer.Version(), peer); err != nil {
		peer.Log().Error("Failed to register peer in pec syncer", "err", err)
		return err
	}
	if snap != nil {
...
// runSnapExtension registers a `snap` peer into the joint pec/snap peerset and
// starts handling inbound messages. As `snap` is only a satellite protocol to
// `pec`, all subsystem registrations and lifecycle management will be done by
// the main `pec` handler to prevent strange races.
...
func (h *handler) removePeer(id string) {
	...
	// Remove the `pec` peer if it exists
...
func (h *handler) Start(maxPeers int) {
	...
	h.wg.Add(2)
	go h.chainSync.loop()
	go h.txsyncLoop64() // TODO(karalabe): Legacy initial tx echange, drop with pec/64.
}
