// Package pec implements the Ethereum protocol.
package pec
...
import (
	...
	"github.com/pecoin/go-fullnode/core/state/pruner"
...
type Ethereum struct {
	...
	// DB interfaces
	chainDb pecdb.Database // Block chain database
...
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	if config.SyncMode == downloader.LightSync {
		return nil, errors.New("can't run pec.Ethereum in light sync mode, use les.LightEthereum")
	}
	...
	chainDb, err := stack.OpenDatabaseWithFreezer("chaindata", config.DatabaseCache, config.DatabaseHandles, config.DatabaseFreezer, "pec/db/chaindata/")
	if err != nil {
		return nil, err
	}
...
func (s *Ethereum) ChainDb() pecdb.Database            { return s.chainDb }
