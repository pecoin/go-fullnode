...
type accountResponse struct {
	...
	nodes pecdb.KeyValueStore // Database containing the reconstructed trie nodes
...
type Syncer struct {
	db    pecdb.KeyValueStore // Database to store the trie nodes into (and dedup)
...
// snap protocol.
func NewSyncer(db pecdb.KeyValueStore, bloom *trie.SyncBloom) *Syncer {
	return &Syncer{
		db:    db,
		bloom: bloom,
...
func (s *Syncer) OnStorage(peer SyncPeer, id uint64, hashes [][]common.Hash, slots [][][]byte, proof [][]byte) error {
	...
	var (
		dbs    = make([]pecdb.KeyValueStore, len(hashes))
