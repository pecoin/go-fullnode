...
type Backend interface {
	...
	// RunPeer is invoked when a peer joins on the `pec` protocol. The handler
	// should do any peer maintenance work, handshakes and validations. If all
	// is passed, control should be given back to the `handler` to process the
	// inbound messages going forward.
	RunPeer(peer *Peer, handler Handler) error

	// PeerInfo retrieves all known `pec` information about a peer.
...
// MakeProtocols constructs the P2P protocol definitions for `pec`.
...
// NodeInfo represents a short summary of the `pec` sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Ethereum network ID (1=Frontier, 2=Morden, Testnet=3)
...
// nodeInfo retrieves some `pec` protocol metadata about the running host node.
...
// Handle is invoked whenever an `pec` connection is made that successfully passes
// the protocol handshake. This method will keep processing messages until the
// connection is torn down.
func Handle(backend Backend, peer *Peer) error {
	for {
		if err := handleMessage(backend, peer); err != nil {
			peer.Log().Debug("Message handling failed in `pec`", "err", err)
			return err
		}
	}
}
