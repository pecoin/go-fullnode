...
import (
	...
	"github.com/pecoin/go-fullnode/internal/jsre"
	"github.com/pecoin/go-fullnode/internal/jsre/deps"
	"github.com/pecoin/go-fullnode/internal/web3ext"
...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"pec": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines pec.getBalance(pec.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
func (c *Console) Welcome() {
	...
	if res, err := c.jsre.Run(`
		...
		try {
			message += "coinbase: " + pec.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + pec.blockNumber + " (" + new Date(1000 * pec.getBlock(pec.blockNumber).timestamp) + ")\n";
		try {
